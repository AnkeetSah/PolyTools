...
const reasons = [
  ...
  return (
    <div className=''>
     <h3 className=' lg:text-lg xs:text-xl font-bold mb-6 text-gray-900 '> Why Choose UltimateUtils</h3>
     <div>
      <div className='grid grid-cols-1 md:grid-cols-4 gap-2'>
        {reasons.map((reason, index) => (
          <div key={index} className='bg-white  p-6 rounded-xl border border-gray-200 '>
            <h4 className='lg:text-lg xs:text-xl font-semibold mb-2 text-gray-900 '>{reason.title}</h4>
            <p className='text-gray-600  lg:text-[12px]'>{reason.description}</p>
          </div>
        ))}
      </div>
     </div>
